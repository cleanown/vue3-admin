<template>
	<div class="full-content" id="revenueChart" @mousemove="stopAutoTooltip" @mouseleave="startAutoTooltip"></div>
</template>
...
const model = defineModel<RevenueInfo[]>()
...
function init() {
	if (!myChart) {
		myChart = echart.init(document.getElementById('revenueChart'))
	}
	myChart.setOption({
		title: {
			text: '营收数据',
		...
		series: [
			{
				...
				// 	}
				// },
				data: model.value.map(item => item.recharge)
			},
			{
				name: '提现',
				type: 'bar',
				stack: 'total',
				smooth: true,
				data: model.value.map(item => item.withdrawal)
			},
			{
				name: '盈利',
				type: 'bar',
				stack: 'total',
				smooth: true,
				data: model.value.map(item => item.profit)
			},
			{
				name: '投放',
				type: 'bar',
				stack: 'total',
				smooth: true,
				data: model.value.map(item => item.delivery)
			},
			{
				name: '余额',
				type: 'bar',
				stack: 'total',
				smooth: true,
				data: model.value.map(item => item.balance)
			},
			{
				name: '其他',
				type: 'bar',
				stack: 'total',
				smooth: true,
				data: model.value.map(item => item.other)
			},
		]
	})
	startAutoTooltip()
